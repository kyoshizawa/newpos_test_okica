...
import jp.mcapps.android.multi_payment_terminal.service.WifiP2pServiceViewModel;
...
public class ViewModelFactory implements ViewModelProvider.Factory {
    ...
    private final Updater _updater = new Updater(_deviceNetworkManager);
    ...
    public <T extends ViewModel> T create(@NonNull Class<T> modelClass) {
        if (modelClass.isAssignableFrom((StartViewModel.class))) {
            return (T) new StartViewModel(_ifBoxManager, _eventLogger, _updater);
        }
        if (modelClass.isAssignableFrom((WifiP2pServiceViewModel.class))) {
            ...
            return (T) new AmountInputViewModel(_ifBoxManager);
        }
        else if (modelClass.isAssignableFrom((MenuViewModel.class))) {
            return (T) new MenuViewModel(_ifBoxManager, _eventLogger);
        }
        else if (modelClass.isAssignableFrom((PeriodicErrorCheckServiceViewModel.class))) {
            return (T) new PeriodicErrorCheckServiceViewModel(_ifBoxManager, _eventLogger);
        }
        else if (modelClass.isAssignableFrom((InstallationAndRemovalViewModel.class))) {
            return (T) new InstallationAndRemovalViewModel(_deviceNetworkManager, _ifBoxManager);
        }
        else if (modelClass.isAssignableFrom((AmountInputLt27ViewModel.class))) {
            return (T) new AmountInputLt27ViewModel(_ifBoxManager);
        }
        else if (modelClass.isAssignableFrom((DriverCodeViewModel.class))) {
            return (T) new DriverCodeViewModel(_tabletLinker);
        }
