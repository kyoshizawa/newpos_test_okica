...
public abstract class DemoValidationCheckDatabase extends RoomDatabase {
    public abstract ValidationCheckDao validationCheckDao();

    private static DemoValidationCheckDatabase _instance;

    public static DemoValidationCheckDatabase getInstance() {
        if (_instance == null) {
            synchronized (DemoValidationCheckDatabase.class) {
                if (_instance == null) {
                    _instance = Room.databaseBuilder(
                            MainApplication.getInstance().getApplicationContext(),
                            DemoValidationCheckDatabase.class,
                            "demo_validation_check")
                            .build();
                }
            }
        }
    ...
    public static void closeDatabase() {
        if(_instance != null) {
            if (_instance.isOpen()) {
                ...
                //インスタンス初期化
                _instance = null;
                Timber.d("Demo Validation Check Database closed");
            }
        }
    }
