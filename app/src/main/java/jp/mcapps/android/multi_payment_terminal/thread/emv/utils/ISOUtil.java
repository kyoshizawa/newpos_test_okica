...
public class ISOUtil {
    ...
    public static byte[] encryptData( byte[]module,byte[]exponents,byte[] clearData)
    {
        KeyFactory keyFactory = null;
        try {
            keyFactory = KeyFactory.getInstance("RSA");
            BigInteger modulus = new BigInteger(byte2hex(module),16);
            BigInteger publicExponent = new BigInteger(byte2hex(exponents),16);
    ...
    public static byte[] int2bcdboth(int data, int len) {
        byte[] bb = null;
        if (len == 1) {
            data = data % 100;
            bb = new byte[1];
            ...
            bb = new byte[2];
            int high=data / 100;
            bb[0] =  (byte) ((((high / 10) % 10) << 4) + (high % 10));
            bb[1] = (byte) ((((data / 10) % 10) << 4) + (data % 10));
            return bb;
        } else
            return null;
    }
    public static byte[] int2byteboth(int data, int len) {
        byte[] bb = null;
        if (len == 1) {
            data = data % 100;
            bb = new byte[1];
    ...
    public static synchronized void Sleep(long miller) {
        ...
        } while(endTime - startTime <= miller);
    }
}
